...
ALTER TABLE public.managed_playlists
  ADD COLUMN IF NOT EXISTS campaign_reserved_slots INT NOT NULL DEFAULT 18;

-- Catálogo puro (arquivadas) = 0; híbridas (ativas) mantém 18
UPDATE public.managed_playlists
   SET campaign_reserved_slots = 0
 WHERE is_catalog = true AND archived_at IS NOT NULL;

UPDATE public.managed_playlists
   SET campaign_reserved_slots = 18
...
BEGIN
  ...
  WITH pool AS (
    SELECT
      ...
      COALESCE((SELECT COUNT(*)::int FROM managed_playlist_tracks mpt WHERE mpt.managed_playlist_id = mp.id), 0) AS real_tracks,
