...
-- 1) Preview: hybrid = campaign_reserved_slots + 1 (FIXO, não append)
...
BEGIN
  ...
  computed AS (
    SELECT
      p.*,
      CASE
        WHEN p.is_hybrid THEN p.campaign_reserved_slots + 1   -- FIXO: insere no topo do bloco de catálogo
        ELSE p.real_tracks + 1                                -- catálogo-puro: append
...
-- 2) Distribute: inclui catálogo-puro arquivado e grava `position` no placement
...
BEGIN
  ...
  inserted AS (
    ...
    SELECT v_track_id, p.managed_playlist_id, 'pending', v_batch_id,
           CASE WHEN p.is_hybrid THEN p.campaign_reserved_slots + 1 ELSE p.real_tracks + 1 END
    FROM pool p
