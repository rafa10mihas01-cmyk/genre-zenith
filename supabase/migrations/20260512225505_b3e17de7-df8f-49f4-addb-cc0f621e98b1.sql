...
BEGIN
  ...
  candidates AS (
    SELECT p.id AS pid, p.name AS pname, p.followers AS pfollowers, p.cover_url AS pcover,
      ...
      -- Capacidade semanal: usa metadata se houver, depois capacity_score*50,
      -- e como ÚLTIMO fallback estima por seguidores (1% por semana / 4).
      COALESCE(
    ...
    WHERE p.ownership IN ('own','curator')
