...
-- 1) Ativa a Distribuição Natural
UPDATE public.system_flags
   SET engine_natural_distribution_active = true,
       updated_at = now()
 WHERE id IN (SELECT id FROM public.system_flags ORDER BY id LIMIT 1);

-- 2) distribute_catalog_track: delega para a Engine Natural quando flag ON
...
DECLARE
  ...
  v_natural_active boolean := false;
  v_plan_id uuid;
BEGIN
  ...
  -- Lê flag da Distribuição Natural
  SELECT COALESCE(engine_natural_distribution_active, false)
    INTO v_natural_active
  FROM public.system_flags ORDER BY id LIMIT 1;

  -- Contagem informativa (mesmo critério das duas rotas)
  WITH pool AS (
    SELECT
  ...
  -- ============================================================
  -- ROTA NOVA — Distribuição Natural por janelas
  -- ============================================================
  IF v_natural_active THEN
    -- O trigger fn_catalog_track_autostart_plan já cria plano em INSERT.
    -- Para faixas pré-existentes ou re-distribuição, garantimos aqui também.
    BEGIN
  ...
  -- ============================================================
  -- ROTA ANTIGA — cap 90 imediato (fallback se flag OFF)
  -- ============================================================
