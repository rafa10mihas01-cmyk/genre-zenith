...
BEGIN
  ...
  IF v_existing IS NULL THEN
    INSERT INTO public.catalog_distribution_plans (
      ...
      'occupancy_engine_full_genre_universe'
    )
  ...
  -- Universo TOTAL do gênero: todas as playlists do catálogo daquele gênero
  -- que ainda não possuem a faixa. Sem filtro de status operacional, modo de
  -- execução, capacidade ou Spotify ID. O Occupancy Engine decide a execução.
  ...
  UPDATE public.catalog_distribution_plans
     SET total_eligible = v_total_targets,
         ...
         next_wave_at = NULL,
         completed_at = v_now,
         notes = 'occupancy_engine_full_genre_universe',
         updated_at = v_now
   WHERE id = v_plan_id;
