-- Fase 5: progresso por música
-- 1) Substitui get_curator_deal_progress aceitando p_song_id opcional
--    e adiciona delivered_per_song no retorno agregado.

DROP FUNCTION IF EXISTS public.get_curator_deal_progress(uuid);
DROP FUNCTION IF EXISTS public.get_curator_deal_progress(uuid, uuid);

CREATE OR REPLACE FUNCTION public.get_curator_deal_progress(
  p_deal_id uuid,
...
BEGIN
  -- Targets: se filtrar por música, usa target da música; senão, do deal.
  ...
  -- delivered_per_song (agregado: sempre calcula sobre todas as músicas do deal)
  WITH all_snaps AS (
  ...
  -- Cálculo principal (filtrado por song quando p_song_id != NULL)
  WITH snaps AS (
