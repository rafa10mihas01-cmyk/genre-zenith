-- Patch get_curator_deal_progress: totais oficiais via fn_deal_delivery_accumulated
...
BEGIN
  ...
  -- per_song (legado: snapshots) — mantido para a tela de detalhe que ainda
  -- consome o breakdown por música.
  WITH curator_pls AS (
  ...
  -- Fonte oficial de delivered_curator / delivered_total: Growth Engine.
  ...
  v_daily_avg := v_engine::numeric / v_days;
  ...
  v_result := jsonb_build_object(
    ...
    'baseline_total', 0,
    'latest_total', v_engine,
    'delivered_curator', v_engine,
    'delivered_total', v_engine,
    'daily_avg', v_daily_avg,
    'days_elapsed', v_days,
    'progress_pct', CASE
      WHEN v_target <= 0 THEN 0
      ELSE LEAST(100, ROUND((v_engine::numeric / NULLIF(v_target,0)::numeric) * 100, 1))
    END,
    'eta_days', CASE
      WHEN v_target <= 0 OR v_engine >= v_target THEN 0
      WHEN v_daily_avg <= 0 THEN NULL
      ELSE CEIL((v_target - v_engine)::numeric / v_daily_avg)
    END,
    'today_plays', NULL,
    'per_playlist', '[]'::jsonb,
