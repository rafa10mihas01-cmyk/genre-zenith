...
CREATE OR REPLACE FUNCTION public.client_approve_campaign(
  ...
  p_approver_ip text DEFAULT NULL::text
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Pré-checagem: status precisa permitir aprovação E plano precisa estar aprovado.
  SELECT id, status, plan_approved_at
    INTO v_campaign_id, v_status, v_plan_approved
  ...
  END IF;
  IF v_plan_approved IS NULL THEN
    RAISE EXCEPTION 'plan_not_approved_yet';
  END IF;
