...
RETURNS TABLE(score numeric, components jsonb, calculated_at timestamp with time zone)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  SELECT EXISTS(
    ...
    WHERE cds.spotify_track_id = v_spotify_track_id
      AND cd.state IN ('active','collecting','approved','running','in_progress')
      AND cd.closed_at IS NULL
      AND (cd.ends_at IS NULL OR cd.ends_at >= v_now)
...
GRANT EXECUTE ON FUNCTION public.compute_placement_priority(uuid) TO authenticated, service_role;
