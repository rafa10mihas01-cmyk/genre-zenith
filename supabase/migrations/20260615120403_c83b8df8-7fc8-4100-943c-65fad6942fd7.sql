...
BEGIN
  ...
  -- rotacionamos: priorizamos playlists que receberam MENOS faixas do catálogo
  -- (carga atual asc) e desempate aleatório. Isso espalha músicas diferentes
  -- por subconjuntos diferentes ao longo do tempo, exercitando todas as playlists.
  WITH pool AS (
    SELECT mp.id AS managed_playlist_id,
           mp.campaign_reserved_slots,
           (mp.archived_at IS NULL) AS is_hybrid,
  ...
  inserted AS (
    ...
    SELECT v_track_id, s.managed_playlist_id, 'pending', v_batch_id,
           CASE WHEN s.is_hybrid THEN s.campaign_reserved_slots + 1 ELSE s.real_tracks + 1 END
    FROM selected s
