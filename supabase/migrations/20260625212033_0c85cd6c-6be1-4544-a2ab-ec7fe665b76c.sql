...
BEGIN
  ...
  WITH pool AS (
    SELECT
      ...
      (
        mp.execution_mode = 'MANUAL_ONLY'
        OR mp.archived_at IS NOT NULL
        OR mp.spotify_playlist_id IS NULL
      ) AS is_ignored
  ...
  SELECT
    COUNT(*)::int,
    COUNT(*) FILTER (WHERE is_ignored)::int,
    COUNT(*) FILTER (WHERE NOT is_ignored AND NOT is_present AND available_slots > 0)::int,
    COUNT(*) FILTER (WHERE NOT is_ignored AND is_present)::int,
    COUNT(*) FILTER (WHERE NOT is_ignored AND NOT is_present AND available_slots <= 0)::int
  INTO v_pool_total, v_ignored_count, v_eligible_total, v_already_present, v_no_capacity
  FROM pool;
