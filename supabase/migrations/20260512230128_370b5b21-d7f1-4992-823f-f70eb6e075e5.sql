CREATE OR REPLACE FUNCTION public.suggest_campaign_playlists(p_goal bigint, p_deadline date, p_exclude_active boolean DEFAULT true, p_only_own boolean DEFAULT false)
...
BEGIN
  ...
  candidates AS (
    ...
    WHERE (CASE WHEN p_only_own THEN p.ownership = 'own' ELSE p.ownership IN ('own','curator') END)
