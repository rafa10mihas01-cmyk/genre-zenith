...
-- Remover overload de 2 args quebrado (ele tentava delegar para função inexistente).
DROP FUNCTION IF EXISTS public.fn_playlist_occupancy_rebuild(uuid, text);

-- Atualizar a versão event-driven (3 args) para usar exclusões operacionais reais,
-- não mais policy_missing como bloqueio.
...
BEGIN
  ...
  -- Exclusões operacionais (regra oficial do universo de distribuição).
  v_block_reason := CASE
    WHEN v_mp.archived_at IS NOT NULL THEN 'archived'
    ...
    WHEN v_mp.execution_mode = 'DISABLED'::playlist_execution_mode THEN 'disabled'
    WHEN COALESCE(v_mp.operational_status,'') = 'do_not_operate' THEN 'do_not_operate'
    ELSE NULL
  END;

  -- Policy editorial: SEMPRE resolvida (playlist -> genre_default -> system_default).
  -- Não é mais usada como filtro de elegibilidade.
