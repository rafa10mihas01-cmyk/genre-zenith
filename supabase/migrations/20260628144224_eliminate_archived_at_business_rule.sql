-- Eliminate archived_at as a business rule across managed_playlists.
-- playlist_type passa a ser a única fonte de verdade operacional.
-- archived_at permanece apenas como timestamp histórico (mantido pelo trigger trg_sync_archived_at_with_playlist_type).
...
BEGIN
  ...
  agg_managed AS (
    ...
    FROM public.managed_playlists WHERE canonical_playlist_id IS NOT NULL AND playlist_type <> 'ARCHIVED'::public.playlist_type_enum
    GROUP BY canonical_playlist_id
  ),
  -- Histórico oficial: vw_campaign_playlist_growth + campaign_eco_allocations
  agg_history AS (
...
BEGIN
  ...
  WHERE genre_id NOT IN (
    ...
    WHERE playlist_type <> 'ARCHIVED'::public.playlist_type_enum AND genre_id IS NOT NULL
  );
...
-- Adiciona playlist_type ao v_catalog_placement_live (mantém archived_at apenas como info histórica)
CREATE OR REPLACE VIEW public.v_catalog_placement_live AS
SELECT cp.id,
  cp.catalog_track_id,
  ...
  mp.playlist_type AS playlist_type,
