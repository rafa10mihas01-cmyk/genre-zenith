CREATE OR REPLACE FUNCTION public.distribute_catalog_track(
  ...
  p_spotify_uri text DEFAULT NULL::text, p_isrc text DEFAULT NULL::text,
  p_track_name text DEFAULT NULL::text, p_artist_name text DEFAULT NULL::text,
...
DECLARE
  ...
  v_max_per_track constant int := 80;
BEGIN
  ...
  -- Cap de 80 playlists por música. Quando o nicho tem mais que 80 elegíveis,
  -- rotacionamos: priorizamos playlists com MENOR carga de catálogo + random.
  --
  -- POSIÇÃO:
  --   * HÍBRIDA (ativa, recebe campanha) → empilha depois da reserva de campanha:
  --       position = campaign_reserved_slots + 1
  --   * ARQUIVADA (catálogo puro) → distribui 50% topo / 25% meio / 25% cauda,
  --       rotacionado por catalog_load da playlist + posição aleatória dentro da zona.
  --       Playlists com menos de 4 faixas reais caem direto pro fim (não dá pra
  --       fatiar em zonas com tão pouca faixa).
  WITH pool AS (
    SELECT mp.id AS managed_playlist_id,
           mp.campaign_reserved_slots,
           (mp.archived_at IS NULL) AS is_hybrid,
           ...
           CASE
             ...
             ELSE
               CASE (s.catalog_load % 4)
                 ...
                 -- 2 = MEIO (25%): pos floor(N/2)+1 .. floor(3N/4)
                 ...
                 -- 3 = CAUDA (25%): pos floor(3N/4)+1 .. N+1
