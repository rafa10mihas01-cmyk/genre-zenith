CREATE OR REPLACE FUNCTION public.create_curator_deal_atomic(
  ...
  p_new_curator jsonb DEFAULT NULL
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Curador: usa o existente OU cria novo dentro da transação.
  ...
  IF v_dup IS NOT NULL AND jsonb_array_length(v_dup) > 0 AND NOT p_force THEN
    -- Levanta exceção para garantir ROLLBACK do curador/compra recém-inseridos.
  ...
  RETURN jsonb_build_object('ok', true, 'deal_id', v_deal_id, 'curator_id', v_curator_id, 'duplicate_warning', v_dup);
END;
$function$;