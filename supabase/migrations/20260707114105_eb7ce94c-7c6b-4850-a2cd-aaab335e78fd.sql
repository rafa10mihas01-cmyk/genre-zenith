...
AS $function$
  WITH xlsx_collections AS (
    SELECT
      c.campaign_id, c.playlist_id, c.playlist_url, c.playlist_name_at_capture,
  ...
  paste_collections AS (
    -- Paste automático do curador. Aceita playlist mesmo que `frozen_at` esteja
    -- setado (congelamento pausa reposição, mas plays já entregues continuam
    -- válidos). Só exclui observacionais e mortas no Spotify.
  ...
  acc AS (
    SELECT c.campaign_id, f.playlist_id, f.delivery_accumulated,
      f.current_reading, f.last_reading_at, f.readings_count, f.last_import_delta
  ...
  SELECT
    ...
    b.baseline_name, b.baseline_plays,
