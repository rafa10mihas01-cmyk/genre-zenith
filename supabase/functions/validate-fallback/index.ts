...
Deno.serve(async (req) => {
  ...
  const async_mode = body.async !== false; // default true
  const job_id = crypto.randomUUID();

  await sb.from("spotify_circuit_breaker").update({ status: "closed", blocked_until: null, retry_after_sec: 0 }).eq("status", "open");

  const report: any = { job_id, started_at: new Date().toISOString(), config: { app05, app01, sul, lado }, phases: {}, rows: [] as any[], created: [] as string[] };
