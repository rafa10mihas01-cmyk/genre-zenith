...
Deno.serve(async (req) => {
  ...
  const { data: allocs, error: aErr } = await supabase
    ...
    .select(
      "id, managed_playlist_id, planned_streams, start_day, status, dispatched_at, position, genre_source, genre_affinity_score, managed_playlists(name, cover_url, followers, spotify_url, genre_id)",
    )
