...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Busca metadata via fetch-spotify-meta (oEmbed + scrape)
    ...
    const name = meta?.title ?? `Playlist ${playlistId}`;
    const cover_url = meta?.thumbnail_url ?? null;
