// Public endpoint: valida código OTP, marca usado, grava log e devolve JWT 24h.
...
Deno.serve(async (req) => {
  ...
  const rl = await checkRateLimit(`verifyCampaignOtp:ip:${ip}`, 60, 30);
  ...
  await supabase
    ...
    .insert({ campaign_id: camp.id, email: emailRaw, ip });

  const jwt = await signAccessJwt({ campaign_id: camp.id, email: emailRaw, token }, 86400);
