...
const PREMIUM_FINISH_BLOCK = [
  ...
  "- Brand signature \"NEXENGINE\": small embossed wordmark in the BOTTOM-RIGHT corner. Position: distance from the right edge and from the bottom edge between 6% and 8% of the canvas (NOT inside the 12% title safe area — the signature lives in the outer corner band, closer to the edge than the text block). Size: width between 6% and 10% of the canvas width. Color: monochromatic — pure white OR a single tone already present in the palette, never a new color. Opacity: between 10% and 15% (very low — barely visible). Finish: faint emboss only (1px soft inner highlight + 1px soft inner shadow, same hue family as the background) so it reads as part of the surface, not a sticker. NO glow. NO drop shadow. NO outline. NO box. NO background plate. Must NEVER compete with the title or subtitle — it is a finishing detail, not a graphic element. Same font family as the title.",
