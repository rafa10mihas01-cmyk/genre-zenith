...
export interface AccessPayload {
  campaign_id: string;
  email: string;
  token: string; // campaign public token (binds JWT to a specific link)
  iat: number;
  exp: number;
}

export async function signAccessJwt(payload: Omit<AccessPayload, "iat" | "exp">, ttlSeconds = 86400): Promise<string> {
  const now = Math.floor(Date.now() / 1000);
  const full: AccessPayload = { ...payload, iat: now, exp: now + ttlSeconds };
...
export async function verifyAccessJwt(token: string): Promise<AccessPayload | null> {
  try {
    ...
    if (!ok) return null;
    const payload = JSON.parse(b64urlDecodeStr(b)) as AccessPayload;
