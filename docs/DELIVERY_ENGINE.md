...
> baseline. Baseline não entrega. Upload diário (`last_24h`/`last_day`) entra
> integralmente. Janela móvel entra apenas pelo crescimento positivo contra a
> leitura anterior.

## Por que este modelo

Os arquivos diários importados como `last_24h`/`last_day` já representam a
entrega daquele dia. Por isso, esses uploads entram integralmente no acumulado.
...
| Estado | Origem |
|---|---|
| `baseline_plays` | `plays_7d` da linha `is_baseline=true`; baseline gera delta 0. |
...
delta_pos =
  ...
  plays_7d                           se window_kind em last_24h/last_day
  plays_7d                           se primeira leitura sem baseline
