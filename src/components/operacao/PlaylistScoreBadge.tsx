    ...
    <TooltipProvider delayDuration={120}>
      <Tooltip>
        <TooltipTrigger asChild>
          <span
            className={cn(
              "inline-flex items-center gap-1 px-1.5 h-5 rounded-md border text-[10px] font-semibold tabular-nums",
              t === "success" && "border-primary/40 bg-primary/15 text-primary",
              t === "warning" && "border-warning/40 bg-warning/10 text-warning",
              t === "danger"  && "border-destructive/40 bg-destructive/10 text-destructive",
              t === "muted"   && "border-border bg-elevated text-muted-foreground",
            )}
          >
            <span className={cn("h-1.5 w-1.5 rounded-full", TONE_BAR[t])} />
            {h}
          </span>
        </TooltipTrigger>
        <TooltipContent side="top" className="text-xs">
          <div className="font-semibold mb-1.5">Health {h}/100</div>
          <div className="grid grid-cols-[auto_60px_auto] gap-x-2 gap-y-1 items-center">
            {rows.map((r) => {
              ...
              return (
                <div key={r.label} className="contents">
                  ...
                  <span className={cn("font-semibold tabular-nums", TONE_TEXT[tt])}>{r.value}</span>
                </div>
              );
            })}
          </div>
