...
async function fetchDetail(id: string) {
  const [trackRes, telemetryRes, placementsRes, attributionRes, snapshotsRes, batchesRes] = await Promise.all([
    ...
    supabase.from("v_catalog_placement_live").select("id, status, current_position, entry_position, added_at, scheduled_for, attempts, last_error_code, managed_playlists:managed_playlist_id(name, cover_url, followers, spotify_playlist_id, archived_at, execution_mode)").eq("catalog_track_id", id).order("status", { ascending: true }),
