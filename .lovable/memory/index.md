...
- [Delivery — arquitetura canônica](mem://architecture/delivery-canonical) — Cadeia oficial pós-FASE 9.2C: 2 writers de snapshot por responsabilidade distinta (auto vs manual), `recompute_campaign_total_delivered` como writer único de cache, `last_reconciled_at` = timestamp do cron, cadeia fn_playlist→fn_curator→fn_campaign→recompute. Consulte antes de tocar qualquer cálculo de entrega.
- [Reorganizar sem remover capacidade](mem://preference/no-remove-without-equivalent) — Antes de remover info de uma tela, validar (rg) que existe em outro local com a mesma fonte. Sem equivalente = só colapsa, nunca remove.
