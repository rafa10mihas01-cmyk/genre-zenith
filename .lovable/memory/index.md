...
Mobile: PageHeader é sticky em `top-14` (logo abaixo do topbar global de 56px). Ações da página rolam horizontalmente para nunca cortar na borda; subtítulo escondido (`hidden md:block`).
Mobile: filtros tipo "chips" e abas em listas (Operação, Criação) ficam em uma linha com scroll-x (`overflow-x-auto`, scrollbar oculto). Tabelas com `grid-cols-12` precisam de fallback em card empilhado para `<md`.
